...
namespace Biblioteca
{
    public static class PDFExtensions
    {
        ...
        public static byte[] GerarTabelaPDF<T>(List<T> obj, string titulo, List<(string Cabecalho, Func<T, string> Valor)> colunas)
        {
            IEnumerable<string> _centralizados = [
                "Data", "Data do Atendimento", "Data de Nascimento", "Telefone", "CPF", "CEP"
                ];
            IEnumerable<string> _numericos = [
            ...
            try
            {
                ...
                foreach (var col in colunas)
                    tabela.AddHeaderCell(new Cell()
                                .Add(new Paragraph(col.Cabecalho).SimulateBold()))
                ...
                foreach (var item in obj)
                {
                    foreach (var col in colunas)
                    {
                        tabela.AddCell(col.Valor(item) ?? "").SetFontSize(10);
                    }
                }
